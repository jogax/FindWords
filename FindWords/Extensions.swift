...
extension String {
    ...
    func countOf(_ myChar: String)->Int {
...
//        }
    ...
    func index(of: String)->Int? {
        for index in 0..<self.length - of.length + 1 {
            if of == self.subString(at: index, length: of.length) {
                return index
            }
        }
        return nil
    ...
    func splitToPreAndPost(with: String)->[String] {
        var returnValue = [String]()
        let emptyString = ""
        if with.length == self.length {
            returnValue.append(emptyString)
            returnValue.append(emptyString)
            return returnValue
        }
        let index = self.index(of: with)
        if index != nil {
            if index == 0 {
                returnValue.append(emptyString)
                returnValue.append(self.endingSubString(at: with.length))
            } else {
                returnValue.append(self.startingSubString(length: index!))
                let postfixIndex = index! + with.length
                if postfixIndex >= self.length {
                    returnValue.append(emptyString)
                } else {
                    returnValue.append(self.endingSubString(at: postfixIndex))
                }
            }
        } else {
            returnValue.append(emptyString)
