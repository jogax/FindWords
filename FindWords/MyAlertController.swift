...
enum AlertType: Int {
    case Gold = 0, Red, Green, White, Clear
}
...
//    let separator = "°"
    ...
    let myClearColor = UIColor(red: 255/255, green: 255/255, blue: 255/255, alpha: 0.4)
...
//    let titleFontName = "AvenirNextCondensed-Bold"
    ...
    init(title: TextConstants, message: TextConstants = .tcEmpty, size: CGSize, target: AnyObject, type: AlertType) {
        ...
        case .Clear: myColor = myClearColor
        }
        
        super.init(texture: nil /*SKTexture(imageNamed: "MenuBG")*/, color: .clear, size: size)
    }
    public func addAction(text: TextConstants, param: Int? = nil, action:Selector, isActive: Bool = false) {
        myTexts.append(GV.language.getText(text, values: param == nil ? "" : String(param!)))
    ...
    public func presentAlert() {
        generateLabels()
        let hMultiplier: CGFloat = 1.1
        let vMultiplier: CGFloat = 1.3
        let newWidth = (self.size.width > ownWidth * hMultiplier) ? ownWidth * hMultiplier : self.size.width
        let newHeight = self.size.height < calculatedHeight * vMultiplier ? self.size.height : calculatedHeight * vMultiplier
        ...
        for (index, label) in myLabels.enumerated() {
            ...
            case messageName:
                if label.text != GV.language.getText(.tcEmpty) {
...
//            let y = self.frame.maxY - ownHeight * CGFloat(index + 1) * (multiplier + 0.04)
            bgIndex = isNormalLine ? bgIndex + 1 : -1
            ...
            if index > countHeaderLines - 2 {
                ...
                myBackgrounds[index].color = activeLines[bgIndex] ? myGrayColor : .clear
    ...
    private func generateLabels() {
        myFontSize = GV.onIpad ? 25 : 25
        titleFontSize = GV.onIpad ? 40 : 25
        messageFontSize = GV.onIpad ? 30 : 25
        ownWidth = 0
...
//        calculatedHeight = ownHeight
    ...
    let lineMultiplier:CGFloat = 1.0
    var firstTitleLine = true
    var firstMessageLine = true
    let titleName = "T"
    let messageName = "M"
